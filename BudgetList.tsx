...
interface BudgetListProps {
  ...
  onCreate?: () => void;
  onEdit: (orçamento: Orçamento) => vazio;
...
const BudgetList: React.FC<BudgetListProps> = ({ budgets: propBudgets, onCreate, onEdit, onDelete, onRefresh }) => {
  ...
  useEffect(() => {
    se (onRefresh) onRefresh();
  }, []);

  const filteredBudgets = useMemo(() => {
    return propBudgets.filter(budget => {      const matchesSearch =
        ...
        (orçamento.númeroDoPedido && orçamento.númeroDoPedido.inclui(termoDeBusca));
  ...
  const getStatusIcon = (status: BudgetStatus) => {
    interruptor (status) {
      caso BudgetStatus.APROVADO: retorne <CheckCircle className="w-4 h-4 text-green-500" />;
      caso BudgetStatus.NOT_APPROVED: retorne <XCircle className="w-4 h-4 text-red-500" />;
      padrão: retornar <Clock className="w-4 h-4 text-yellow-500" />;
    }
  };

  const getStatusBadgeClass = (status: BudgetStatus) => {
    interruptor (status) {
      caso BudgetStatus.APPROVED: retornar 'bg-green-100 text-green-700';
      caso BudgetStatus.NOT_APPROVED: retorne 'bg-red-100 text-red-700';
      padrão: retornar 'bg-amarelo-100 texto-amarelo-700';
    }
  };
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {Object.values(BudgetStatus).map(status => (
              <option key={status} value={status}>{status opção>
            ))}
          </select>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Dados</th>
...
exportar lista de orçamento padrão;
